...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    if (specialCallbackParam === "true") {
      setSpecialModalOpen(true);
      const newParams = new URLSearchParams(searchParams.toString());
      newParams.delete("callbackSpecial");
      const newUrl = `${pathname}?${newParams.toString()}`;
      router.replace(newUrl, { scroll: false });
    }
  }, [searchParams]);
    ...
    >
      ...
      <div className="container mx-auto px-4 py-3 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-2">
          <Image src="/logopng.png" alt="Logo" width={40} height={40} className="sm:w-14 sm:h-14 object-fill" />
        ...
        <nav className="hidden lg:flex items-center gap-4">
          <Link href={`/${localeNav}/about`} className="hover:text-[#1e7bff] text-sm">{t("about")}</Link>
          <Link href={`/${localeNav}`} className="hover:text-[#1e7bff] text-sm">{t("home")}</Link>
          <Link href={`/${localeNav}/apartments`} className="flex items-center bg-blue-900 rounded-3xl p-2 gap-1 hover:text-[#1e7bff] text-sm">
            <span>{t("all_apartments")}</span>
          </Link>
        </nav>

        <button className="lg:hidden p-2" onClick={toggleMobileMenu} aria-label="Open menu">
      ...
      {isMobileMenuOpen && (
        <nav className="lg:hidden bg-[#0e1e3a] px-4 py-4 border-t border-gray-700">
          <div className="flex flex-col gap-4">
            <Link href={`/${localeNav}/about`} className="hover:text-[#1e7bff] text-sm">{t("about")}</Link>
            <Link href={`/${localeNav}`} className="hover:text-[#1e7bff] text-sm">{t("home")}</Link>
            <Link href={`/${localeNav}/apartments`} className="hover:text-[#1e7bff] text-sm">{t("all_apartments")}</Link>
          </div>
        </nav>
      )}

      <CallbackModal isOpen={modalOpen} onClose={() => setModalOpen(false)} />
      <CallbackModal isOpen={specialModalOpen} onClose={() => setSpecialModalOpen(false)} limitedCountries={true} />
    </header>
  );
}
